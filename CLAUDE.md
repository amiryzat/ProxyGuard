...
A standalone, read-only Flask app (`dashboard/app.py`, port 5001) that reads `logs/attendance.csv` and `logs/snapshots/` and never writes to either.
- **Present / Attempts tabs**: the table is split into two tabs on one page (client-side toggle, no separate routes) — Present shows `result=success` rows for the selected session, Attempts shows `result=failed` rows, each with its own summary stat boxes (total/successful/failed/flagged) computed only from that tab's currently visible rows.
- **Session filter** (dropdown, defaults to most recent session) and a **reason filter** scoped to the Attempts tab (dropdown of the distinct `reason` values among the selected session's failed rows, defaulting to "All reasons") apply together — both live in one `<form>` so changing either resubmits both, with a hidden field preserving which tab was open.
- **Inline verification snapshots**: each row shows a thumbnail of its matched snapshot file (click to open full-size) or a "no snapshot" placeholder. Matching (`find_snapshot_for_row()`) rebuilds the snapshot filename prefix from the row's `session_id`/`name`/`reason` (mirroring `build_snapshot_filename()` in `src/main.py`); if that strict match fails (e.g. an older row where the CSV `name` and the snapshot's name segment diverge — see `docs/bugs.md` Bug 8), it falls back to matching on `session_id` + `reason` + the row's own timestamp, ignoring the name segment.
- **Flagged row highlighting** reuses `pattern_flagger.flag_patterns()` (no duplicated detection logic), same as before.
...
│   ├── main.py             # CheckinSession: shared check-in engine (recognition, liveness, multi-face + identity-continuity guards, logging, snapshots) + the desktop entry point that drives it (done)
...
│   ├── app.py              # reads attendance CSV + snapshots, session + reason filters, Present/Attempts tabs, flagged-row highlighting (reuses pattern_flagger.py), per-tab summary, inline snapshot matching
│   └── templates/
│       └── index.html
├── checkin_app/            # standalone Flask student check-in web app: setup page + live MJPEG check-in (done — see docs/architecture.md)
│   ├── app.py              # setup form -> builds session_id via class_config.build_session_id() -> starts a CheckinSession -> live check-in page (MJPEG stream, status polling, reset)
│   └── templates/
│       ├── setup.html
│       ├── confirm.html    # Phase 2a leftover, no longer used by /start
│       └── checkin.html    # live check-in page: session header, MJPEG <img>, status line, "New check-in" button
├── logs/
│   ├── attendance.csv
...
- Dashboard (Flask) is now built (`dashboard/`, Step 7 done, plus Phase A/B/C additions — see Core System Design section 8). Liveness checks, attendance logging, pattern flagging, and the lecturer dashboard all work end to end; only testing/refinement (Step 8) remains of the planned steps.
...
- Student check-in web app (`checkin_app/`) now implements both the Phase 2a setup page and the Phase 2b live check-in flow (camera streamed into the browser as MJPEG, driven by the shared `CheckinSession`) — see Core System Design section 7. Both `src/main.py`'s webcam loop and `checkin_app/` are real check-in entry points now, sharing one engine.
