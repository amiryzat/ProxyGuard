# src/visualize_liveness_metrics.py
# Standalone chart generator for the liveness evaluation numbers already
# reported in docs/report.md (TAR, FRR_liveness, average check-in latency).
# Not wired into main.py/checkin_app/dashboard on purpose -- this only
# re-visualizes numbers that were already computed elsewhere, it doesn't
# read any live CSV or camera data.
#
# Run: python3 src/visualize_liveness_metrics.py
# Saves docs/liveness_metrics_charts.png and opens an interactive window.
...
def plot_metrics():
    ...
    # 3. Average check-in latency, single bar
    ax = axes[2]
